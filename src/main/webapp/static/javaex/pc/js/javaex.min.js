		...
		var info = {
			...
			comment : function(args) {
				...
				for (var i = 0; i < list.length; i++) {
					...
					html += '<span style="color:#959595;">' + list[i][commentMapping.time] + '</span>';var replyList = list[i][commentMapping.replyInfoLsit];
					if (userId == "" || userId == null) {
					} else {
				...
				scoreChange = function(elem, index) {
					return $(elem).each(function(i, scoreElem) {
						return $(scoreElem).find("i").each(function(i, item) {
							if (i <= index) {
								$(item).removeClass("icon-star_border");return $(item).addClass("icon-star")
							} else {
								$(item).removeClass("icon-star");return $(item).addClass("icon-star_border")
							}
						})
					})
				};
			...
			optTip : function(args) {
				...
				if (type == "submit") {
					...
					if ($(".tip-icon-loading").length > 0) {
						$(".opt-tip").css("background-color", "#ff6e6e");$(".opt-tip .tip-icon").removeClass("tip-icon-loading");$(".opt-tip .tip-icon").addClass("icon-x");$(".opt-tip .tip-icon").css({
							"font-size" : "16px",
							"font-weight" : "bold"
						});$(".opt-tip .tip-msg").text(opt.content)
					} else {
			...
			menu : function(args) {
				...
				$("#" + menuId + " .menu-item>a").on("click", function() {
					if ($(this).next().css("display") == "none") {
						...
						$(this).next("ul").slideUp(300);$(".menu-item.menu-show").removeClass("menu-show")
					}
				});$("#" + menuId + " .menu-item ul li").click(function() {
					addHover($(this))
				});
				function addHover(obj) {
					$("#" + menuId + " li.menu-item").removeClass("hover");$("#" + menuId + " .menu-item ul li").removeClass("hover");obj.addClass("hover")
				}
				...
				function menuSelected(url) {
					$("#" + menuId + " li").each(function() {
						...
						if (url.indexOf(href) >= 0) {
							flag = true;setCookie("menuUrl", url);
							if ($(this).hasClass("menu-item")) {
								$("#menu li.menu-item").removeClass("hover");$("#menu .menu-item ul li").removeClass("hover");$(this).addClass("hover")
							} else {
			...
			date : function(args) {
				...
				function removeCSS() {
					...
					for (var d = new Date(bDate); d.getTime() <= ced.getTime(); d.setDate(d.getDate() + 1)) {
						choice = "choice-style";$("#date_" + date2ymd(d).join("-")).removeClass(choice);$("#date_" + date2ymd(d).join("-")).removeClass("started").removeClass("ended").removeClass("selected")
					}
				}
				function addCSS() {
					if (monthNum == 1) {
					...
					for (var d = new Date(startDate); d.getTime() <= endDate.getTime(); d.setDate(d.getDate() + 1)) {
						choice = "choice-style";$("#date_" + date2ymd(d).join("-")).removeClass("started").removeClass("ended").removeClass("selected");$("#date_" + date2ymd(d).join("-")).removeClass(choice);$("#date_" + date2ymd(d).join("-")).attr("class", choice)
					}
					$("#date_" + date2ymd(new Date(startDate)).join("-")).removeClass().addClass("started");$("#date_" + date2ymd(new Date(endDate)).join("-")).removeClass().addClass("ended")
				}
				function selectDate(ymd) {
					isClear = false;var ymdFormat = formatDate(ymd);
					if (this.dateInput == ("hide-date-" + dateId)) {
						removeCSS();$("#date_" + ymd).attr("class", "selected");$("#" + this.dateInput).val(ymdFormat);return
					}
					if ($("#" + this.dateInput).val() == "") {
						if (this.dateInput == ("hide-start-" + dateId)) {
							removeCSS();$("#date_" + ymd).attr("class", "selected");$("#" + this.dateInput).val(ymdFormat);
				...
				function fillDate(year, month, index) {
					...
					for (var d = dateBegin; d.getTime() <= dateEnd.getTime(); d.setDate(d.getDate() + 1)) {
						...
						if (deviation == 0) {
							ymd = d.getFullYear() + "-" + (d.getMonth() + 1) + "-" + d.getDate();$(td).attr("id", "date" + "_" + ymd);$(td).css("cursor", "pointer");(function(ymd) {
			...
			edit : function(args) {
				...
				editHtml += '</div>';
				editHtml += '</div>';$("#" + editId).append(editHtml);$("#" + editId + " .edit-body-container").bind("mouseup keyup", function() {
