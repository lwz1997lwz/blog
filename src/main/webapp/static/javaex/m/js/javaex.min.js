	...
	var javaex = function() {
		...
		var info = {
			...
			tip : function(args) {
				...
				tipHtml += '</div>';
				tipHtml += '</div>';
				tipHtml += '</div>';$(document.body).append(tipHtml);setTimeout(function() {
			...
			loading : function(args) {
				...
				loadingHtml += '</div>';
				loadingHtml += '</div>';$(document.body).append(loadingHtml)
			},
			...
			dialog : function(args) {
				...
				if (type == "html") {
					...
					$("#" + UUID).before('<div class="mask"></div>');$("#" + UUID).css("transform", "translateY(0px)");$(".mask").click(function() {
						$(".mask").remove();$("#" + UUID).css("transform", "translateY(100%)");setTimeout(function() {
							info.close(UUID)
						}, 300)
					})
				}
			...
			close : function(UUID) {
				...
				if (ogMask != null) {
					ogMask.remove();
					oDialog.style.bottom = -(oDialog.offsetHeight) + "px";oDialog.classList.remove("fadeInUp");oDialog.classList.add("fadeInDown");setTimeout(removeDialog, 300)
				} else {
					oDialog.remove();
			...
			nav : function(args) {
				...
				if (opt.type == "slide") {
					...
					$("#slide-nav").before('<div class="mask"></div>');$("#slide-nav").css("transform", "translateX(" + $("#slide-nav").width() + "px)");$(".mask").click(function() {
						$(".mask").remove();$("#slide-nav").css("transform", "translateX(0px)")
					})
				} else if (opt.type == "guide") {
					if ($("#guide-nav > ul").hasClass("active")) {
						$("#guide-nav > ul").removeClass("active");$("#guide-nav ul").css("height", "1rem")
					} else {
						var liCount = $("#guide-nav > ul > li").length;
						var row = Math.ceil((liCount + 1) / 5);
						$("#guide-nav > ul").addClass("active");$("#guide-nav ul.active").css("height", (row + 0.2) + "rem")
					}
				}
			},
			roll : function(args) {
			...
			page : function(args) {
				...
				info.windowHeight = document.documentElement.clientHeight;init();
				function init() {
					...
					info.loadDistance = Math.floor($("#infinite-scroll-preloader-" + pageId).height() * 1 / 3);$(window).on('scroll', function() {
			...
			loadDown : function(pageId) {
				...
				info.isDataLoaded = true;info.loadDataFunction()
			},
			...
			tab : function(args) {
				...
				$("#" + tabId + " ul li").css("width", (100 / tabNum) + "%");$("#" + tabId + " ul .border").css("width", (100 / tabNum) + "%");$("#" + tabId + " ul li").each(function(i) {
					if (i == (current - 1)) {
						$(this).addClass("current");$("#" + tabId + " .border").css("left", $(this).offset().left + "px")
					}
					...
					$(this).addClass("current").siblings().removeClass("current");$("#" + tabId + " .border").css("left", $(this).offset().left + "px");$(".tab-content>div:eq(" + $(this).index() + ")").show().siblings().hide();opt.callback({
						"index" : $(this).index() + 1
					})
				})
			},
			date : function(args) {
				...
				dateHtml += '</div>';
				dateHtml += '</div>';
				dateHtml += '</div>';$(document.body).append(dateHtml);$("#date-box-" + dateId + " table ul").after('<div class="mask-data"><div class="mask-up"></div><div class="mask-mid"></div><div class="mask-down"></div></div>');
				var isStart = true;
				var isMove = false;
				var isEnd = false;
				var startY = 0;
				...
				$("#" + dateId).bind("click", function() {
					init();$("#date-box-" + dateId).show();return
				...
				function init() {
					$("#year-" + dateId + " ul").empty();$("#month-" + dateId + " ul").empty();$("#day-" + dateId + " ul").empty();$("#year-" + dateId + " ul").html(yearList);$("#month-" + dateId + " ul").html(monthList);$("#day-" + dateId + " ul").html(dayList);var date = $("#final-date-value-" + dateId).val();
					...
					day = (now.getDate() < 10 ? "0" : "") + now.getDate();var curMonthDays = new Date(year, month, 0).getDate();
				...
				for (var i = 0; i < oScrollList.length; i++) {
					oScrollList[i].addEventListener("touchstart", function(event) {
					...
					}, false);oScrollList[i].addEventListener("touchmove", function(event) {
						event.preventDefault();
						...
						if (isMove) {
							nowElement.css("transition", "none");nowElement.css("transform", "translate(0, " + -(nY - (mY - startY)) + "px)")
						}
					...
					}, false);oScrollList[i].addEventListener("touchend", function(event) {
						event.preventDefault();
						...
						if (isEnd) {
							...
							nowY = endY;
							if (nY > maxY) {
								nowElement.css("transition", "all .5s");nowElement.css("transform", "translate(0, " + maxY + "px)")
							} else if (nY < minY) {
								nowElement.css("transition", "all .5s");nowElement.css("transform", "translate(0, " + minY + "px)")
							} else {
									...
									function inertiaMove() {
										...
										if (dir * nowV > 0) {
											if (totalDistance > maxY) {
												...
												nowElement.css("transition", "all .5s");nowElement.css("transform", "translate(0, " + Math.round(totalDistance / liHeight) * liHeight + "px)")
											}
											setTimeout(function() {
										...
										if (totalDistance > (maxY + (liHeight * 2))) {
											nowElement.css("transition", "all .5s");nowElement.css("transform", "translate(0, " + maxY + "px)");return
										} else if (totalDistance < (minY - (liHeight * 2))) {
											nowElement.css("transition", "all .5s");nowElement.css("transform", "translate(0, " + minY + "px)");return
										}
										nowElement.css("transform", "translate(0, " + totalDistance + "px)");setTimeout(function() {
											fillDate()
										}, 500);setTimeout(inertiaMove, 10)
									}
									inertiaMove()
								})(v, endTime, nY)
							}
						...
						if (id == "year-" + dateId || id == "month-" + dateId) {
							...
							if (dif > 0) {
								...
								if (moveY > (curMonthDays - 1 - 2) * liHeight) {
									$("#day-" + dateId + " .date-ul").css("transition", "all 0s");$("#day-" + dateId + " .date-ul").css("transform", "translate(0, " + -(curMonthDays - 1 - 2) * liHeight + "px)");
									day = curMonthDays
								}
							}
						}
					}
					$("#final-date-text-" + dateId).html(year + "-" + month + "-" + day);$("#final-date-value-" + dateId).val(year + "-" + month + "-" + day)
				}
			...
			select : function(args) {
				...
				selectHtml += '</div>';
				selectHtml += '</div>';
				selectHtml += '</div>';$(document.body).append(selectHtml);$("#select-box-" + selectId + " ul").after('<div id="mask-data-' + selectId + '" class="mask-data"><div class="mask-up"></div><div class="mask-mid"></div><div class="mask-down"></div></div>');
				var isStart = true;
				var isMove = false;
				var isEnd = false;
				var startY = 0;
				...
				var selectName = "";
				init();close(true);$("#input-" + selectId).bind("click", function() {
					init();$("#select-box-" + selectId).show();return
				...
				function init() {
					$("#opt-select-" + selectId + " ul").empty();$("#opt-select-" + selectId + " ul").html($("#" + selectId).html());$("#opt-select-" + selectId + " option").addClass("select-option option-show");$("#opt-select-" + selectId + " option").each(function() {
				...
				oScroll.addEventListener("touchstart", function(event) {
					...
					lastMoveTime = new Date().getTime();
					stopInertiaMove = true
				}, false);oScroll.addEventListener("touchmove", function(event) {
					event.preventDefault();
					mY = event.touches[0].clientY;
					if (!isStart) {
						isMove = true;
				...
				}, false);oScroll.addEventListener("touchend", function(event) {
					event.preventDefault();
					endY = event.changedTouches[0].clientY;
				...
				if (keyword == "") {
					...
					$("#opt-select-" + selectId + " ul option").each(function(i) {
						if ($(this).text().toLowerCase().indexOf(keyword.toLowerCase()) == -1) {
							$(this).removeClass("option-show").addClass("option-hide");count++
						} else {
							$(this).removeClass("option-hide").addClass("option-show");indexArr.push(i)
						}
					});
					if (indexArr != "" && indexArr != null) {
						...
						info.lastIndex = indexArr[indexArr.length - 1];$("#opt-select-" + selectId + " .select-ul").css("transform", "translate(0, " + -(info.firstIndex - 2) * liHeight + "px)");
